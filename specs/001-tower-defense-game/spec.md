...
- **FR-001**: System MUST display a game area with a circular wall surrounding a central town; wall has visible HP and level indicators (Level 1: starts at 400 HP)
- **FR-002**: System MUST spawn enemies from any direction that move toward the circular wall; enemies attack immediately upon reaching the wall (Level 1: single enemy every 3 seconds)
- **FR-002a**: Initial wall strength MUST be sufficient to survive at least 50% of Level 1 wave duration (30+ seconds) without any player resource management or healing
- **FR-003**: System MUST provide three resource generation buttons (Blue, Green, Yellow) that generate resources at base rate of 1.0 resource/second when activated
- **FR-004**: System MUST allow wall upgrades using Blue resources that increase wall level and maximum HP by +100 HP per level, and visual appearance (Level 1: 400 HP, base cost: 50 Blue)
- **FR-005**: System MUST provide healing functionality using Green resources that restores current HP up to maximum at 1:1 ratio (1 Green = 1 HP; Level 1 base cost: 30 Green restores 30 HP)
- **FR-006**: System MUST allow Yellow resource upgrades that increase generation rates by +0.5 resources/second per upgrade for any resource type (base: 1.0/sec; Level 1 upgrade cost: 40 Yellow)
...
- **FR-009**: System MUST scale resource costs exponentially with game level progression at ~1.5x multiplier per level (e.g., Level 1 heal: 30 Green → Level 2: 45 Green → Level 3: 68 Green)
...
- **FR-013**: Enemies MUST deal damage to the wall at a consistent rate when they reach it (Level 1: 10 damage per attack at 1-second intervals)
...
- **NFR-011**: Game difficulty curve MUST provide appropriate challenge progression (Level 1: tutorial-level difficulty with single enemy spawns at intervals; wall survives 30+ seconds idle)

### Key Entities

- **Wall**: Circular defensive structure surrounding central town with HP, level, maximum HP, visual appearance that changes with upgrades
- **Enemy**: Attacking units that spawn from any direction and move toward wall at 100 pixels/second (Level 1); attack immediately upon contact; have speed, damage, HP attributes that scale with game level
...
- Q: Initial gameplay balance and difficulty curve → A: Level 1 starts easy with single enemy spawning at intervals; initial wall strength survives at least 50% of level time without user interaction
- Q: Base resource generation rate when player first activates generation (before upgrades) → A: 1.0 resource per second
- Q: Enemy damage per attack and attack interval at Level 1 → A: 10 damage per attack at 1-second intervals (10 DPS)
- Q: Enemy spawn interval at Level 1 → A: 3-second intervals (20 enemies per minute)
- Q: Initial wall HP at Level 1 → A: 400 HP (survives ~40 seconds idle with 20-second learning window)
- Q: Base costs for primary actions at Level 1 → A: Wall upgrade: 50 Blue | Heal: 30 Green | Generation upgrade: 40 Yellow
- Q: Spatial layout and enemy spawn/attack mechanics → A: Wall is circular surrounding a central town; enemies spawn from any direction and attack wall immediately upon reaching it
- Q: Enemy movement speed at Level 1 → A: 100 pixels/second (moderate pace with ~5-7 second spawn-to-wall window)
- Q: Healing amount per Green resource → A: 1:1 ratio (1 Green = 1 HP restored)
- Q: Wall HP increase per upgrade level → A: +100 HP per upgrade level (Level 1: 400 HP → Level 2: 500 HP)
- Q: Generation rate upgrade effect → A: +0.5 resources/second per upgrade (1.0→1.5→2.0→2.5, etc.)
- Q: Cost scaling formula across game levels → A: Exponential scaling at ~1.5x multiplier per level (Level 2: 1.5x, Level 3: 2.25x, Level 4: 3.4x)
